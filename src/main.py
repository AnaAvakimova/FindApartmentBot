...
from menu import (
    first_confirm_deletion,
    first_confirm_registration,
    get_main_menu_markup,
    second_confirm_deletion,
...
USER_STATE: Dict[int, str] = {}  # dictionary for tracking user state
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("first_confirm_registration")
)
def callback_first_confirm_registration(call):
    ...
    if call.data == "first_confirm_registration_yes":
        ...
        bot.send_message(
            call.from_user.id,
            "Какой у вас номер квартиры? Только номер, без текста и пробелов:",
        )
...
@bot.message_handler(
    func=lambda message: get_user_state(message) == "awaiting_apartment"
)
    ...
    try:
        ...
        if apartment_number not in range(1, 536):
            bot.send_message(
                ...
                "Пожалуйста, введите корректный номер квартиры (от 1 до 535)",
            ...
            username = (
                ...
                else f"[нажмите для связи](tg://user?id={user_id})"
            )
        ...
        bot.send_message(
            message.from_user.id, second_mess, reply_markup=get_main_menu_markup()
        )


@bot.callback_query_handler(
    func=lambda call: call.data.startswith("second_confirm_registration")
)
...
def reg_user(user_id, name, username, apartment_number):
    ...
    try:
        ...
        print(
            f"Номер квартиры в базе {user_check_apt}, номер введенной квартиры {apartment_number}"
        )
        ...
        if user_check_apt is not None:
            for apt in user_check_apt:
                ...
                if apartment_number == apt[0]:
                    ...
                    bot.send_message(
                        user_id, second_mess, reply_markup=get_main_menu_markup()
                    )
                    return
...
def check_user(message):
    ...
    try:
        ...
        if cursor.fetchone() is None:
            ...
            message = (
                "Только зарегистрированные пользователи могут получать данные о соседях"
            )
...
@bot.message_handler(
    func=lambda message: get_user_state(message) == "checking_apartment"
)
def check_apartment(message):
    ...
    try:
        ...
        cursor.execute(
            "SELECT name, username FROM Users WHERE apartment = ?", (apartment_number,)
        )
        ...
        cursor.execute(
            "UPDATE Users_check SET count_check = count_check + 1 WHERE user_id = ?",
            (user_id,),
        )
        connection.commit()
        cursor.execute(
            "SELECT count_check FROM Users_check WHERE user_id = ?", (user_id,)
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("first_confirm_deletion")
)
def callback_confirm_deletion(call):
    ...
    if call.data == "first_confirm_deletion_yes":
        ...
        bot.send_message(
            call.from_user.id,
            "Укажите номер квартиры для удаления. Только номер, без текста:",
        )
...
@bot.message_handler(
    func=lambda message: get_user_state(message) == "delete_registration"
)
def delete_registration(message):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM Users WHERE user_id = ? and apartment = ?",
            (
                user_id,
                apartment_number,
            ),
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("second_confirm_deletion")
)
...
def check_membership(message):
    ...
    if not is_user_member(group_id, user_id):
        bot.reply_to(
            message,
            "Извините, вы не являетесь участником группы и не можете использовать этот бот",
        )
    else:
...
def process_user_message(message):
    ...
    if message.text == "/start":
        ...
        first_mess = (
            f"<b>{message.from_user.first_name}</b>, привет!\nЧто вы "
            f"хотите сделать?"
        )
        bot.send_message(
            message.chat.id,
